<?php
  class UserReg_Model extends CI_Model{
    ...
    public function get_userBy_id($id){
      ...
      ->where('id',$id);
      $q=$this->db->get();
      //the lower comment will give us the std object at the given index
      // so that we donot have to pass the array
      // when we need to update the data so row is more efficient
    // return $q->row();
